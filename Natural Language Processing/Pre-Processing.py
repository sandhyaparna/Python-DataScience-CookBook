...
Df["Text_Var2"] = Df["Text_Var"].apply(lambda x: ' '.join([word for word in x.split() if word not in (stop_words)]))
...
# Removes stop words from the Text var - Text_Var1 has words seperated by comas
 Df["Text_Var1"] = Df["Text_Var"].str.lower().str.split() -- Same as Tokenization
...
Df["Text_Var3"] = Df["Text_Var"].apply(nltk.word_tokenize)
